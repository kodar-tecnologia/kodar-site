...
export default function PortfolioCard({
    ...
    return (
        <button
            type="button"
            onClick={() => setFlipped((s) => !s)}                // toque (mobile)
            aria-pressed={flipped}
            aria-label={`Ver detalhes de ${title}`}
            title={title}
            className={`group relative h-56 sm:h-60 md:h-64 lg:h-72 w-full overflow-hidden
            ...
            >
                ...
                <div className="absolute inset-0 [backface-visibility:hidden]">
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-3 sm:p-4">
                        ...
                        <h3 className="mt-2 text-lg sm:text-xl font-semibold text-white">{title}</h3>
                        {subtitle && <p className="text-xs sm:text-sm text-gray-300">{subtitle}</p>}
                    </div>
                </div>

                {/* Verso */}
                <div
                    className="absolute inset-0 rounded-2xl border border-white/10 bg-[#0b1420]/95 p-3 sm:p-4
                     [transform:rotateY(180deg)] [backface-visibility:hidden]"
                >
                    <h3 className="text-base sm:text-lg font-semibold text-white">{title}</h3>
                    {subtitle && <p className="text-xs sm:text-sm text-gray-400">{subtitle}</p>}

                    <ul className="mt-3 space-y-2 text-[13px] sm:text-sm text-gray-300">
                        {bullets.map((b, i) => (
                    ...
                    {/* leve gradiente no rodapé para leitura */}
